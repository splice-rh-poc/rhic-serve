...
class RHICRcsApiTest(MongoApiTestCase):
    ...
    def test_get_deleted_rhics(self):
        ...
        response = self.get('/api/rhicrcs/?deleted__exact=True')
        json = response.content
        rhics = simplejson.loads(json)
        self.assertEquals(1, len(rhics))

        # Delete 2 more rhics
        self.delete('/api/rhic/ed74e2a5-eb37-4bcb-9504-a9c338db56d0/')
        self.delete('/api/rhic/01720e7a-14d0-4a30-9ea9-de11c50c1d31/')

        # Should only be 6 non-deleted rhics left
        response = self.get('/api/rhicrcs/')
        json = response.content
        rhics = simplejson.loads(json)
    ...
    def test_get_modified_rhics(self):
        ...
        response = self.get(
            '/api/rhicrcs/?modified_date__gt=2012-09-14T21:50Z')
        json = response.content
        rhics = simplejson.loads(json)
        ...
        # Modify a rhic
        response = self.patch(
            '/api/rhic/47decdb7-d2eb-4918-b28c-0c5f57de8c19/',
